...
export default class AddVisite extends Component {
  ...
  constructor(props) {
    ...
    this.saveOrUpdateIntervention = this.saveOrUpdateIntervention.bind(this);
}
...
componentDidMount(){
    ...
    // step 4
    if(this.state.id === '_addI'){
        return
    }else{
      InterventionDataService.getInterventioneById(this.state.id).then( (res) =>{
...
saveOrUpdateIntervention = (e) => {
    ...
    // step 5
    if(this.state.id === '_addI'){
      InterventionDataService.createIntervention(intervention).then(res =>{
...
getTitle(){
    if(this.state.id === '_addI'){
        return <h3 className="text-center">Add Intervention</h3>
    }else{
        return <h3 className="text-center">Update Intervention</h3>
    }
}
render() {
    return (
