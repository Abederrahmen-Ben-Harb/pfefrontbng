...
export default class DemandesList extends Component {
  ...
  constructor(props) {
    ...
    this.editDemande = this.editDemande.bind(this);
    this.deleteDemande = this.deleteDemande.bind(this);
}

deleteDemande(id){
  DemandeDataService.deleteDemande(id).then( res => {
        this.setState({demandes: this.state.demandes.filter(demande => demande.id !== id)});
    });
}
viewDemande(id){
    this.props.history.push(`/view-demande/${id}`);
}
editDemande(id){
    this.props.history.push(`/add-demande/${id}`);
}

componentDidMount(){
  DemandeDataService.getDemandes().then((res) => {
        this.setState({ demandes: res.data});
    });
}

addDemande(){
    this.props.history.push('/add-demande/_addD');
}
...
render() {
    return (
        <div>
             ...
             <div className = "row">
                <button className="btn btn-primary" onClick={this.addDemande}> Add Demande</button>
             </div>
             <br></br>
             <div className = "row">
                        ...
                        <tbody>
                            {
                                this.state.demandes.map(
                                    ...
                                    <tr key = {demande.id}>
                                         ...
                                         <td> { demande.dateRetour} </td>
                                         <td> {demande.etat}</td>
                                         <td>
                                             <button onClick={ () => this.editDemande(demande.id)} className="btn btn-info">Update </button>
                                             <button style={{marginLeft: "10px"}} onClick={ () => this.deleteDemande(demande.id)} className="btn btn-danger">Delete </button>
                                             <button style={{marginLeft: "10px"}} onClick={ () => this.viewDemande(demande.id)} className="btn btn-info">View </button>
                                         </td>
                                    </tr>
                                )
                            }
