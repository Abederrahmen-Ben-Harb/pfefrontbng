...
export default class PlantesList extends Component {
  ...
  constructor(props) {
    ...
    this.addPlante = this.addPlante.bind(this);
    this.editPlante = this.editPlante.bind(this);
    this.deletePlante = this.deletePlante.bind(this);
}

deletePlante(id){
  PlanteDataService.deletePlante(id).then( res => {
        this.setState({plantes: this.state.plantes.filter(plante => plante.id !== id)});
    });
}
viewPlante(id){
    this.props.history.push(`/view-plante/${id}`);
}
editPlante(id){
    this.props.history.push(`/add-plante/${id}`);
}

componentDidMount(){
    PlanteDataService.getPlantes().then((res) => {
        this.setState({ plantes: res.data});
    });
}

addPlante(){
    this.props.history.push('/add-plante/_addP');
}

render() {
    return (
        <div>
             <h2 className="text-center">Plantes List</h2>
             <div className = "row">
                <button className="btn btn-primary" onClick={this.addPlante}> Add Plante</button>
             </div>
             <br></br>
             <div className = "row">
                    <table className = "table table-striped table-bordered">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th> Type du Plante </th>
                                <th> Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {
                                this.state.plantes.map(
                                    plante => 
                                    <tr key = {plante.id}>
                                         ...
                                         <td> {plante.type}</td>
                                         <td>
                                             <button onClick={ () => this.editPlante(plante.id)} className="btn btn-info">Update </button>
                                             <button style={{marginLeft: "10px"}} onClick={ () => this.deletePlante(plante.id)} className="btn btn-danger">Delete </button>
                                             <button style={{marginLeft: "10px"}} onClick={ () => this.viewPlante(plante.id)} className="btn btn-info">View </button>
                                         </td>
                                    </tr>
                                )
                            }
